/* ********************************************************************************************
 *  modification of PKT for printing the lan party passcode
 *  This is extremely ugly code that is more C than C++. It could be made better by an 
 *  order of magnitude, but this should suffice for the challenge.
 *  This is also extremely overkill: peeling the non-truss edges should work.
 *  See https://louridas.github.io/rwa/assignments/finding-trusses/
 *
...
int load_graph_from_stdin(graph_t *g) {

    string line;
    while (getline(cin, line)) {
        auto split_pt = line.find("-");
        string s1 = line.substr(0, split_pt);
        string s2 = line.substr(split_pt+1);
    ...
    verts = vector<string>(vertset.begin(), vertset.end());
    ...
    for (auto [u, v] : edges) {
        m++;
        g->num_edges[vert_map[u]]++;
        g->num_edges[vert_map[v]]++;
    }
    ...
    for (const auto& [su, sv] : edges) {
        int u = vert_map[su], v = vert_map[sv];
...
void display_stats(int *EdgeSupport, Edge *edgeIdToEdge, long numEdges) {
    int minSup = INT_MAX;    
    int maxSup = 0;

    for(long i = 0; i < numEdges; i++) {
	    maxSup = max(maxSup, EdgeSupport[i]);
    }

    long numEdgesWithMaxSup = 0;

    set<int> maxtruss_vertset;

    for(long i = 0; i < numEdges; i++) {
        if( EdgeSupport[i] == maxSup ) {
            maxtruss_vertset.insert(edgeIdToEdge[i].u);
            maxtruss_vertset.insert(edgeIdToEdge[i].v);
            numEdgesWithMaxSup++;
        }
    }
...
#pragma omp parallel for 
    ...
    display_stats(EdgeSupport, edgeIdToEdge, g.m /2); 
